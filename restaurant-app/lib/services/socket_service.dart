...
final newOrderStreamProvider = StreamProvider<Map<String, dynamic>>((ref) {
  final socketService = ref.watch(socketServiceProvider);
  return socketService.onOrderNew;
});
...
/// Handles restaurant-specific events including order management
class SocketService {
  IO.Socket? _socket;
  ...
  // Restaurant ID for room management
  ...
  void _setupEventListeners() {
    ...
    _socket!.onConnect((_) {
      ...
      if (_restaurantId != null) {
        _socket!.emit('join:restaurant', _restaurantId);
      }
    });
    ...
    _socket!.on('restaurant:order_received', (data) {
      if (data != null) {
        _orderNewController.add(_parseData(data));
      }
    });
    ...
    _socket!.on('restaurant:order_cancelled', (data) {
      if (data != null) {
        _orderCancelledController.add(_parseData(data));
      }
    });
  ...
  void acceptOrder(String orderId, {int? estimatedTime}) {
    if (_socket != null && _socket!.connected) {
      _socket!.emit('order:accepted', {
        'orderId': orderId,
