...
/// Edit menu item screen state
class EditMenuItemState {
  final MenuItem? item;
...
/// Edit menu item notifier
class EditMenuItemNotifier extends StateNotifier<EditMenuItemState> {
  final MenuService _menuService;

  EditMenuItemNotifier(this._menuService) : super(const EditMenuItemState());
  ...
  Future<void> loadItem(String itemId) async {
    ...
    try {
      final results = await Future.wait([
        _menuService.getMenuItemById(itemId),
        _menuService.getCategories(),
      ]);
      state = state.copyWith(
        item: results[0] as MenuItem,
        categories: results[1] as List<MenuCategory>,
        isLoading: false,
      );
    } catch (e) {
      state = state.copyWith(
...
/// Edit menu item provider
final editMenuItemProvider = StateNotifierProvider.autoDispose
    .family<EditMenuItemNotifier, EditMenuItemState, String>((ref, itemId) {
  final menuService = ref.watch(menuServiceProvider);
  final notifier = EditMenuItemNotifier(menuService);
...
class _EditMenuItemScreenState extends ConsumerState<EditMenuItemScreen> {
  ...
  void _initializeFields(MenuItem item) {
    ...
    _discountEndsAt = item.discountEndsAt;
          ...
          children: [
            ...
            if (_existingImageUrl != null || _imageFile != null)
              ListTile(
                ...
                title:
                    const Text('ازالة الصورة', style: TextStyle(color: AppColors.error)),
                onTap: () {
                  setState(() {
                    _imageFile = null;
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () async {
                                final edited = await _showOptionDialog(context,
                                    option: opt);
                                if (edited != null) {
  ...
  Future<void> _saveItem() async {
    ...
    final notifier = ref.read(editMenuItemProvider(widget.menuItemId).notifier);
    ...
    final item = await notifier.updateMenuItem(
  ...
  Future<void> _deleteItem() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('هل انت متأكد من حذف هذا الصنف؟ لا يمكن التراجع عن هذا الاجراء.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('الغاء'),
    ...
    if (confirmed == true && mounted) {
      final notifier =
          ref.read(editMenuItemProvider(widget.menuItemId).notifier);
      final success = await notifier.deleteItem(widget.menuItemId);
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(editMenuItemProvider(widget.menuItemId));
    final notifier = ref.read(editMenuItemProvider(widget.menuItemId).notifier);

    // Initialize fields when item is loaded
    if (state.item != null && !_isInitialized) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _initializeFields(state.item!);
        setState(() {});
      });
    }

    // Listen for errors
    ref.listen<EditMenuItemState>(
        editMenuItemProvider(widget.menuItemId), (previous, next) {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'حذف',
            onPressed: state.isDeleting || state.isSaving ? null : _deleteItem,
          ),
          TextButton.icon(
            onPressed: state.isSaving || state.isDeleting ? null : _saveItem,
            icon: state.isSaving
                ? const SizedBox(
                    width: 16,
                    height: 16,
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('فشل تحميل الصنف'),
                      const SizedBox(height: 16),
                      ElevatedButton(
                        onPressed: () => notifier.loadItem(widget.menuItemId),
              ...
              : Stack(
                  children: [
                    Positioned(
                      ...
                      child: CircleAvatar(
                        radius: 16,
                        backgroundColor: AppColors.grey800.withValues(alpha: 0.7),
                        child: IconButton(
            ...
            return Card(
              child: ListTile(
                ...
                leading: Icon(
                  ...
                  color:
                      addon.isAvailable ? AppColors.success : AppColors.grey400,
                ),
                trailing: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
