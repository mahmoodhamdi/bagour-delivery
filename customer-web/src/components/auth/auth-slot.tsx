...
/**
 * Header right-side widget. When signed in: avatar + dropdown menu. When
 * signed out: Sign in CTA. Hidden until the auth store hydrates from
 * localStorage so we don't flash an anonymous state on cold reload.
 */
export function AuthSlot() {
  ...
  const handleSignOut = async () => {
    try {
      ...
      // Server may return 401 if our token already expired — we still clear locally.
    }
    clear();
